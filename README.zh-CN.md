...
- **主归档实验**：M1-M5 的全数据与 16-shot 结果。
- **低样本主归档实验**：M4/M5 的 4-shot、8-shot 结果。M1/M2/M3 的低样本实验尚未补齐，因此在汇总表中保留空白。
...
- 额外的补充与探索性实验也已单独归档，其中包括若干 M5 额外变体，以及两组 CLC 结果；后者保留了产物，但当前仓库快照中没有对应的 `train_clc.py`。
...
**如何解读 M5 结果**：核心判据仍然是 `M5_best > max(M2_best, M4_best)`，但应该按具体设置分别判断，而不是当成统一规律。当前仓库快照中，支持 M5 互补性的主要证据集中在 4-shot、8-shot 以及匹配设置的 8-shot 多种子验证。
...
`results/tables/` 下的汇总表是当前仓库快照的结果真值来源。下面统一使用**归档 best checkpoint** 的指标，而不是最后一个 epoch 的数值。
...
| **M4 LoRA (r=4)** | **79.46%** | **147,456** | 当前归档中最强的 16-shot 结果 |
| M5 CoOp→LoRA | 79.26% | 155,648 | 在该设置下略低于 M4 |
...
**按当前归档产物能支持的结论**：M5 在极低样本场景下很有价值，但清理后的全数据和 16-shot 结果**不支持**“M5 一定优于 M4”的泛化说法。在这个仓库快照里，M4 在 16-shot 和全数据上更强；M5 的优势主要体现在 4-shot、8-shot，以及匹配设置下的 8-shot 多种子验证。

### 小样本覆盖矩阵

空白项表示实验还没补齐，先留空等待后续直接续填。
...
### 4-shot 与 8-shot 主结果对比（M4 vs. M5）

| 设置 | M4 LoRA | M5 CoOp→LoRA | 增益 |
|------|--------:|-------------:|-----:|
| 4-shot  | 30.70% | **55.40%** | **+24.70 pp** |
| 8-shot  | 43.08% | **67.78%** | **+24.70 pp** |
| 16-shot | **79.46%** | 79.26% | −0.20 pp |

在主归档结果中，混合方法在 4-shot 和 8-shot 上优势非常大，到 16-shot 基本消失。
