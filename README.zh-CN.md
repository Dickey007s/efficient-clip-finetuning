...
**按当前归档产物能支持的结论**：在三个已收敛的低样本设置（4-、8-、16-shot）上，M5 都小幅超过参数匹配的 M4 LoRA 基线，且样本越少优势越大（8-shot +0.71 pp，16-shot +2.21 pp，4-shot +3.78 pp）。这种互补性在全数据上并**不**成立——全数据下 M4（97.25%）仍优于 M5（96.48%）。结论是：CoOp 预热恰恰在数据稀缺、文本锚点更关键时帮助最大，而当视觉编码器拥有足够样本可独立适配后，这种增益随之消失。
