...
- **主归档实验**：M1-M5 的全数据结果、M1-M4 的 16-shot 结果、M4/M5 的 8-shot 结果，以及 M5 的 4-shot 结果。
- **待补低样本实验**：M1/M2/M3 的 4-shot 与 8-shot、M4 的 4-shot、M5 的 16-shot。没有收敛到可支撑结论的结果先留空。
...
**如何解读 M5 结果**：核心判据仍然是 `M5_best > max(M2_best, M4_best)`，但应该按具体设置分别判断，而不是当成统一规律。最终 epoch 仍明显上升、轮数不足的结果不进入主结论。当前仓库快照中，可用的正向证据集中在已收敛的 4-shot M5、8-shot 对比，以及匹配设置的 8-shot 多种子验证。
...
| M5 CoOp→LoRA | 待补 | 待补 | 已归档的 10-epoch LoRA 阶段仍在上升，因此排除出主结果 |
...
**按当前归档产物能支持的结论**：M5 在已收敛的低样本实验中有价值，但清理后的全数据结果**不支持**“M5 一定优于 M4”的泛化说法。M5 的 16-shot 当前仍是待补项，因为已有产物轮数不足、不能支撑主结果结论。
...
|------|--------:|-------------:|-----------|
| 4-shot  | 待补 | **69.16%** | M4 短轮次已废弃 |
| 8-shot  | 73.85% | **74.56%** | **+0.71 pp** |
| 16-shot | **79.46%** | 待补 | M5 短轮次已废弃 |

旧的 4-shot/8-shot 短轮次数值不再作为主结果使用，因为训练曲线还没有收敛到足以支撑论述。当前能直接比较 M4 与 M5 的最强归档主结果是 8-shot；结果只显示 M5 有小幅优势，和后面的多种子表一致。4-shot 的 M4 和 16-shot 的 M5 先留空等待长轮次补跑。
