...
| M1 线性探测 | 47.51% | 约 22 K | +20.0% | 仅头部适配 |
| M2 CoOp | **67.01%** | 约 8 K | **+39.5%** | 文本端优化 |
| M4 LoRA (r=4) | 79.46% | 约 147 K | +51.9% | 视觉端低秩适配 |
| **M5 CoOp→LoRA** | **80.58%** | **约 156 K** | **+53.1%** | **顺序混合（ours）** |

**核心发现**：M5 仅使用全数据 2.6% 的训练样本（688 vs 26640），达到 **80.58%**，超过全数据线性探测（74.09%），同时参数量仅为 M3 CLIP-Adapter 的 **1/7**。

**互补效应验证**（单种子，16-shot）：

```text
M5 相对 CoOp 增益 = 80.58% - 67.01% = +13.57%  （显著）
M5 相对 LoRA 增益  = 80.58% - 79.46% = +1.12%   （较小但为正）
```

M5 同时优于两种单独方法，表明在 16-shot GTSRB 设置下，文本端 prompt 适配与视觉端低秩适配具有互补效应。相对 CoOp 的增益显著，相对 LoRA 的增益较小但为正。LoRA 阶段的 Delta vs Stage1 从 +2.88% 单调递增至 +15.14%，无震荡或过拟合。

> **注意**：上述结果为单随机种子。多种子验证有助于确认混合增益的稳健性，尤其是相对 LoRA-only 的小幅优势。

### 全数据（26,640 张训练样本）

| 方法 | 测试集 Top-1 | 可训练参数量 | 说明 |
|------|--------------|--------------|------|
| M1 线性探测 | 74.09% | 约 22 K | 全数据头部调优 |
| M5 CoOp→LoRA | 待补充 | 约 156 K | 顺序混合 |
...
预期行为：若 `M5c_best > M5d_best`，则对 GTSRB 而言"先文本锚点、后视觉适配"优于"先视觉适配、后文本锚点"。
