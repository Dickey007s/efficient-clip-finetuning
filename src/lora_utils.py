...
"""
lora_utils.py
LoRA 注入工具：为 open_clip ViT 的 attention (QKV + output) 和 MLP 完整注入 LoRA。
...
方案：
  用自定义 AttentionWithLoRA 替换每个 resblock 的 attn，内部用独立 q/k/v/out
  Linear 并各挂一个 LoRA 分支；MLP 的 c_fc/c_proj 也挂 LoRA。所有原始权重从
  原 MHA 拷贝过来并冻结，只训练 LoRA 分支。这样保证：
    1) 注入前数学等价于原 CLIP（LoRA 初值 B=0，增量为 0）
    2) Q/K/V/out/MLP 都能被 LoRA 适配
...
class AttentionWithLoRA(nn.Module):
    ...
    def forward(self, query, key, value, need_weights=False, attn_mask=None):
        """
        与 nn.MultiheadAttention(batch_first=True) 等价。
