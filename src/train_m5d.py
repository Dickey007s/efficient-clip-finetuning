...
"""
train_m5d.py
M5d: LoRA→CoOp (反向顺序消融)

Stage 1: LoRA warm-up — 冻结 CLIP，只训练 image encoder 的 LoRA (Q/V)
Stage 2: CoOp fine-tuning — 冻结 Stage 1 的 LoRA，只训练 learnable prompt

验证: 先视觉适配再文本适配，是否不如先文本锚点再视觉微调。

用法:
  python src/train_m5d.py --lora_epochs 20 --coop_epochs 10 --lr 0.002 --lora_lr 1e-4 \
      --batch_size 64 --n_ctx 16 --rank 4 --alpha 8 --save outputs/m5d_full.pt
"""
import os
...
def train_epoch_features(model, train_loader, optimizer, scaler, criterion):
    """Stage 2 CoOp: 训练使用 feature loader。"""
    model.train()
...
def main():
    ...
    parser.add_argument("--lora_epochs", type=int, default=20, help="Stage 1: LoRA warm-up epochs")
    parser.add_argument("--coop_epochs", type=int, default=10, help="Stage 2: CoOp fine-tuning epochs")
    parser.add_argument("--lr", type=float, default=0.002, help="Stage 2 CoOp LR")
    parser.add_argument("--lora_lr", type=float, default=1e-4, help="Stage 1 LoRA LR")
    ...
    print("=" * 60)
    print("M5d: LoRA→CoOp (Reversed Order Ablation)")
    print(f"LoRA epochs: {args.lora_epochs} | CoOp epochs: {args.coop_epochs}")
    print(f"LoRA LR: {args.lora_lr} | CoOp LR: {args.lr} | Batch: {args.batch_size}")
    print(f"Shots: {args.shots or 'full'} | N_CTX: {args.n_ctx} | LoRA rank: {args.rank} | alpha: {args.alpha}")
    print("=" * 60)

    # 加载数据（原始 image loader，Stage 1 需要）
    ...
    print(f"\n{'='*60}")
    print("Stage 1: LoRA Warm-up (training visual Q/V LoRA)")
    print(f"{'='*60}")
    ...
    if args.lora_ckpt is not None:
        ...
        print(f"[Checkpoint] Warm-start Stage 1 accuracy before training: {best_acc:.2f}%")
    ...
    print(f"[Stage 1] Best LoRA accuracy: {best_acc:.2f}%")

    # 保存 Stage 1 的 LoRA
    lora_path = Path(args.save).with_suffix('').with_name(Path(args.save).stem + '_stage1_lora.pt')
    ...
    print(f"[Stage 1] LoRA weights saved to {lora_path}")

    # ========== Stage 2: CoOp Fine-tuning ==========
    # 冻结 LoRA，解冻 prompt
    ...
    print(f"\n{'='*60}")
    print("Stage 2: CoOp Fine-tuning (LoRA frozen)")
    print(f"{'='*60}")

    # 记录 Stage 1 最终准确率（作为 Stage 2 的基准）
    stage1_final_acc = evaluate_acc(model, image_test_loader)
    print(f"[Stage 1] Final frozen-LoRA accuracy before CoOp: {stage1_final_acc:.2f}%")

    # 预计算 image features（Stage 2 CLIP + LoRA 都冻结）
    ...
    # 训练结束后只调用一次详细评估
    acc, confusion, per_class_acc, failures = evaluate_detailed(model, feature_test_loader)
    ...
    print(f"M5d Gain over LoRA:     {gain:+.2f}%")
    print("=" * 60)
