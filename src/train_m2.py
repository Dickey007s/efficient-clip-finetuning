...
class CoOpModel(nn.Module):
    ...
    def forward(self, image):
        ...
        # 手动过 text encoder
        # 1. 加 positional embedding (open_clip 新版 transformer 是 batch-first)
        x = prompts + self.clip_model.positional_embedding.to(prompts.dtype)

        # 2. Transformer (batch-first: [N, L, D])
        ...
        x = x[torch.arange(x.shape[0], device=x.device), eot_indices]  # [n_cls, ctx_dim]

        # 5. Text projection
        if self.clip_model.text_projection is not None:
            if isinstance(self.clip_model.text_projection, nn.Linear):
                text_features = self.clip_model.text_projection(x)
            else:
